...
type RiwayatItem = Prisma.BarangKeluarGetPayload<{
	include: {
...
export function TabelBarangKeluar({
	data,
	total,
	page,
	pageSize,
	...
	const updateParams = useCallback(
		(updates: Record<string, string | undefined>) => {
			...
			router.push(`/dashboard/barang-keluar?${params.toString()}`)
		},
		[router, searchParams],
	)
			...
			<div className="flex flex-col sm:flex-row gap-3 justify-between">
				...
				<Dialog open={open} onOpenChange={setOpen}>
					...
					<DialogContent className="max-w-lg">
						...
						<FormBarangKeluar barangList={barangList} onSuccess={() => setOpen(false)} />
					</DialogContent>
				</Dialog>
			</div>
						...
						{data.length === 0 ? (
							...
							data.map((item, index) => (
								<TableRow key={item.id} className="hover:bg-surface-raised">
									...
									<TableCell className="text-sm text-right font-medium text-danger">
										−{item.jumlah}
									</TableCell>
